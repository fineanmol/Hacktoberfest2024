...
def logger(text):
    """
    Logs activity in the account to `_bank_log_v1.txt`
    :param text: The text in the log
    :return: `text` to log
    """
    with open("C:\\Users\\Public\\Documents\\_bank_log_v1.txt", "a") as log:
        print(text, file=log)
...
if accountName in userNamesDataBase:
    passwords = bytes.decode(userNamesDataBase[accountName], encoding='utf-8')
    accountPassword = bytes(accountPassword, encoding='utf8')
    if accountPassword in base64.standard_b64decode(passwords):
        accountNumbers = userAccountNumber[accountName]
        accountNumber = bytes.decode(accountNumber)
