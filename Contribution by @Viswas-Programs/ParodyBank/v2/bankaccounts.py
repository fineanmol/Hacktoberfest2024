...
ageOfAccount = shelve.open("C:\\Users\\Public\\Documents\\permissions")

childMoneyMax = 5000
kidWithdrawMax = 2000
current_Time = datetime.datetime.strftime(datetime.datetime.now(), "%Y %m %d")
if current_Time > "2022 05 20":
...
def logger(text):
    """
    Logs activity in the account to `_bank_log_v2.txt`
    :param text: The text in the log
    :return: `text` to log
    """
    with open("C:\\Users\\Public\\Documents\\_bank_log_v2.txt", "a") as log:
        print(text, file=log)
...
class Account:
    ...
    def ShowBalance(self):
        """
        Show the balance of `__init__(name)`
        :return: Balance of `__init__(name)
        """
        return self.balance
...
print("SBI official program to access money")
...
userNamesDataBase = shelve.open("C:\\Users\\viswa\\PycharmProjects\\OOP\\"
                                "Bankaccountfiles\\bankAccountNames")
userAccountNumber = shelve.open("C:\\Users\\viswa\\PycharmProjects\\OOP\\"
