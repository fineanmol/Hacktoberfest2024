...
int prefix_convert(char infix[],char pf_array[]){
    char stack[20];
    int pre_size = 0;
    int size = 0;
    int flag=0;
        ...
        switch(infix[i]){
            ...
            while (flag==0){
                if (stack[size-1]!='^'){
                    ...
                    pf_array[pre_size] = pop(stack,size);
                    size = size-1;
                    pre_size = pre_size+1;
                }
            }
...
int prefix_evaluate(char postfix[]){
    int int_stack[50];
    int size=0;
    int i,mid;
    for (i=0;postfix[i]!='\0';i++){
        switch (postfix[i]){
            ...
            break;
            case '^':
            mid = power(int_pop(int_stack,size),int_pop(int_stack,size-1));
            size = size-2; 
            int_push(int_stack,size,mid);
            size = size+1;
            break;
...
int main(){
    ...
    while (choice!=3){
        ...
        switch (choice){
            ...
            scanf("%s",infix);
            printf("---Converting to Pretfix---\n");
            reverse(infix);
            prefix_convert(infix,prefix);
            reverse(prefix);
