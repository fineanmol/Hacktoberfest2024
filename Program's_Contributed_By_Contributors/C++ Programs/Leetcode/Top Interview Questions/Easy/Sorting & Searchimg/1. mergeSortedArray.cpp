...
public:
    void merge(vector<int>& nums1, int m, vector<int>& nums2, int n) {
        int i = m - 1, j = n - 1; 
        //i starts as the last element of nums1 and j starts as the last element of nums2
        while (i >= 0 && j >= 0) { 
            int k = i + j + 1; 
            //k >= 0 and k < nums1.size(). k is the index of the current element in the destination array. 
