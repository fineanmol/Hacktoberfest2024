...
# Enable Windows Store in Windows 10/11 (1903+)

# Enable Windows Store
Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsStore" -Name "AutoDownload" -Value 0 -Type DWord -Force

# Enable Windows Store Background Tasks (Windows 10/11 1903+)
Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsStore" -Name "DisableBackgroundTasks" -Value 0 -Type DWord -Force

# Enable Windows Store Auto Update (Windows 10/11 1903+) 
Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsStore" -Name "DisableOSUpgrade" -Value 0 -Type DWord -Force
